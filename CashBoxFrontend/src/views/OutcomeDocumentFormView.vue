  ...
  <div class="page-card wide-card">
    ...
    <form class="entity-form" @submit.prevent="saveDocument">
      <div class="form-header-grid">
        <label>
          ...
          <select v-model="form.supplierId" required>
            ...
            <option v-for="s in suppliers" :key="s.id || s.Id" :value="s.id || s.Id">
              {{ s.fullName || s.code || s.name }}
            </option>
          </select>
        </label>
        <label>
      ...
      <div class="items-section">
        <h3>Mahsulotlar tarkibi {{ isEdit ? '(Tahrirlash uchun qatorni 2 marta bosing)' : '' }}</h3>
...
const loadDocument = async () => {
  ...
  try {
    ...
    error.value = 'Ma\'lumot yuklashda xatolik yuz berdi.';
  }
};

const addItem = () => form.value.tables.push({ productId: null, quantity: 1, price: 0 });
const removeItem = (index) => form.value.tables.splice(index, 1);
const calculateTotalSum = computed(() => form.value.tables.reduce((s, i) => s + (parseFloat(i.quantity) * parseFloat(i.price)), 0));
...
onMounted(async () => {
  await loadMetaData();
  await loadDocument();
  if (form.value.tables.length === 0 && !isEdit.value) addItem();
});
</script>
